...
args = {
    ...
    "device": "cuda",
    #测试改ceshi
    "data_path": "/root/pytorch_rt1_with_trainer_and_tester-11148a1d43ad3a0757d69114acbdd9c7b78f84d0/dataset_400/",
    #测试改ceshi
...
#测试改ceshi
args["resume_from_checkpoint"] = "/root/autodl-tmp/tea_box_test/box-9-8-(all)-data-400/19-checkpoint.pth"
args["resume"] = True
if args["resume"]:
...
class TouchTaskEnv(SimEnv):
    ...
    def load_env(self):
        ...
        #测试改ceshi,要自己上传urdf文件到那个服务器上
        self.tar_obj = p.loadURDF("urdf/pink_tea_box/model.urdf", [0, 0, 0], globalScaling=0.8)
        p.changeVisualShape(self.tar_obj, -1, rgbaColor=None)
        self.robot = Panda()
        self.robot.load()
...
results = []
#测试改ceshi，视频保存的路径要改
video_dir = "/root/autodl-tmp/tea_box_test/box-9-8-(all)-data-400/videos/"
...
#测试改ceshi，路径要改
excel_path = "/root/autodl-tmp/tea_box_test/box-9-8-(all)-data-400/(all)-data-400-19pth-results.xlsx"
