...
# command1_que="一级指令是对象不明确（不应该包含需要抓取的物体类别以及需要抓取的具体物体名字）或者操作不明确的指令。例子有：我准备将桌面上的电脑带走请将电脑上的物体处理好、我希望盒子里是空的、整理桌面。生成一级指令的思考步骤：1、提取物体及其特征：从图片中提取出所有物体，并结合图片和世界通用知识描述物体的属性特征（必须包括物体的情况、颜色、位置），在该步骤中输出提取出的物体名字以及物体详细的特征描述。2、思考图片中有什么需要整理。3、根据图片和物体描述，结合通用知识生成一条一级指令。请你一步一步思考。"
# extract_command1_que="请从以上描述中提取出一级指令,不需要对描述中的一级指令做任何更改，只需要输出一级指令，不需要输出其他内容"
command1_que="A level-one command is an instruction where the object is not clearly specified (it should not include the category of objects to be grabbed or the specific names of the objects to be grabbed) or the operation is unclear. Examples include: I’m going to take the computer from the desk, please handle the objects on the computer; I want the box to be empty; organize the desk. The thought process for generating a level-one command: 1. Extract objects and their features: Extract all objects from the image and describe the attributes and characteristics of the objects by combining the image and common knowledge (must include the condition, color, and location of the objects). In this step, output the names of the extracted objects and a detailed description of their characteristics. 2. Consider what needs to be organized in the image. 3. Based on the image and object descriptions, combine with common knowledge to generate a level-one command. Please think step by step."
...
if __name__ == '__main__':
    ...
    image_paths1 = image_paths[800:900]
    ...
    for image in image_paths:
        ...
        # 实时保存Excel文件
        wb.save("G:\\xjtu\\llm\\project\\command\\output\\output_8_14_command1_generate_zhengli.xlsx")

    # 最终保存Excel文件（可选，因为每次写入后已保存）
    wb.save("G:\\xjtu\\llm\\project\\command\\output\\output_all_8_14_command1_generate_zhengli.xlsx")
