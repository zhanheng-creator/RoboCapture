...
# command1_que="一级指令是对象不明确（不应该包含需要抓取的物体类别以及需要抓取的具体物体名字）或者操作不明确的指令。例子有：我口渴了请将合适的物体放到盒子里、我饿了把我需要的东西放盒子里、我需要维修设备帮我在盒子里准备好我需要的物品。生成一级指令的思考步骤：1、提取物体及其特征：从图片中提取出所有物体，并结合图片和世界通用知识描述物体的属性特征（必须包括物体的用途、物体的情况、颜色、位置），在该步骤中输出提取出的物体名字以及物体详细的特征描述。2、根据图片和物体描述，结合物体的用途与通用知识推理在什么情况需要图中某些物品，生成一条一级指令（一级指令需要包含需要使用某些物品的情景）。请你一步一步思考。"
# extract_command1_que="请从以上描述中提取出一级指令,不需要对描述中的一级指令做任何更改，只需要输出一级指令，不需要输出其他内容"
command1_que="A level-one command is an instruction where the object is not clearly specified (it should not include the category of objects to be grabbed or the specific names of the objects to be grabbed) or the operation is unclear. Examples include: I'm thirsty, please put the appropriate object into the box; I'm hungry, put what I need into the box; I need to repair the equipment, help me prepare the items I need in the box. The thought process for generating a level-one command: 1. Extract objects and their features: Extract all objects from the image and describe the attributes and characteristics of the objects by combining the image and common knowledge (must include the object's purpose, condition, color, and location). In this step, output the names of the extracted objects and a detailed description of their characteristics. 2. Based on the image and object descriptions, combine the object's purpose with common knowledge to infer in what situation certain items in the image would be needed, and generate a level-one command (the level-one command should include a scenario where certain objects are needed). Please think step by step."
...
if __name__ == '__main__':
    ...
    image_paths1 = image_paths[400:600]
    ...
    for image in image_paths:
        ...
        # 实时保存Excel文件
        wb.save("G:\\xjtu\\llm\\project\\command\\output\\output_8_14_command1_generate_tuili.xlsx")

    # 最终保存Excel文件（可选，因为每次写入后已保存）
    wb.save("G:\\xjtu\\llm\\project\\command\\output\\output_all_8_14_command1_generate_tuili.xlsx")
