...
if __name__ == '__main__':
    ...
    for i in range(2,501):
        ...
        for command3 in command3s:
            # 保存结果到Excel文件
            sheet.append(
                [image.replace("G:\\xjtu\\llm\\project\\command\\data_new\\rgb1\\", ""), command1,
                 assistant_dismantle_res,command3])

            # 实时保存Excel文件
            wb.save("G:\\xjtu\\llm\\project\\command\\output\\output_8_14_command1_all_dismantle.xlsx")

    # 最终保存Excel文件（可选，因为每次写入后已保存）
    wb.save("G:\\xjtu\\llm\\project\\command\\output\\output_all_8_14_command1_all_dismantle.xlsx")
